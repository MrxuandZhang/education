 ...
 * layui.cascader({
        ...
        //     "Msg": "错误！！"
        // },
        init:function($elem , $cascader){

        },
        onShow:function($elem , $cascader){

        },
        onHide:function($elem , $cascader){
...
layui.define(["jquery","laytpl","layer"], function (exports) {
    ...
    Cascader.prototype={
        ...
        init: function () {
            var $elem = $(this.elem);
            $elem.parent().find(".layui-icon layui-icon-down").remove();
            $elem.parent().find(".urp-cascader-content").remove();
            $elem.after('<i class="layui-icon layui-icon-down"></i>');
            $elem.after('<div class="urp-cascader-content"><div class="cascader-list"></div><div class="cascader-floot"><div style="text-align: center;"><div class="btn btn-xs btn-white cascader-btn">确认</div></div></div></div>');
            var $urpContent = $elem.parent().find('div.urp-cascader-content');
            this.$urpCascaderContent = $urpContent;
            this.$cascaderList = this.$urpCascaderContent.find('div.cascader-list');
            this.$cascaderFloot = this.$urpCascaderContent.find('div.cascader-floot');
            var height = window.innerHeight;
            var marginTop = $urpContent.css('margin-top');
            if(marginTop.indexOf('px') > 0){
                marginTop = parseFloat(marginTop);
            }else{
                marginTop = parseFloat(marginTop) * height;
            }
            var top  = $elem.height() + marginTop;
            $urpContent.css({'top':top+'px'});
            this.initFloot();
            if($.isFunction(this.option.init)){
                this.option.init($elem , this.$urpCascaderContent);
            }
        },
        /**
         * 初始化底部操作
        ...
        initFloot : function(){
            var self = this;
            /* 初始化按钮事件 */
            this.$cascaderFloot.find('.cascader-btn').on('click' , function () {
                self.finishInitData()
            });
        },
        ...
        initFirst: function () {
            ...
            this.$cascaderList.append(string);
            this.$urpCascaderContent.hide();

            // 显示隐藏第一层的标签
            for(var i=0;i<this.d.length;i++) {
                if (this.d[i].children && this.d[i].children.length) {
                    this.$cascaderList.find("ul.urp-cascader-child li").eq(i).find("i").show()
                }else{
                    this.$cascaderList.find("ul.urp-cascader-child li").eq(i).find("i").hide()
                }
            }
        },
        // 获取当前点击的当前面板的数据
        ...
        initChild: function (triggerData) {
            // 删除后面的面板
            this.$cascaderList.find(".urp-cascader-child:gt("+(this.floor)+")").remove();
            // 获取text值
            ...
            for(var i=0;i<this.blockData["children"].length;i++){
                ("children" in this.blockData["children"][i])?(
                    this.$cascaderList.find("ul.urp-cascader-child:gt("+(this.floor)+")").find("li").eq(i).find("i").show()
                ):(
                    this.$cascaderList.find("ul.urp-cascader-child:gt("+(this.floor)+")").find("li").eq(i).find("i").hide()
                );
            }
        ...
        finishInitData: function (triggerData) {
            ...
            this.onOff = false;
            this.$urpCascaderContent.slideUp(100);

            if($.isFunction(this.option.onHide)){
                this.option.onHide($(this.elem) , this.$urpCascaderContent);
            }
            this.$urpCascaderContent.find('i.layui-icon-up').replaceWith('<i class="layui-icon layui-icon-down"></i>');
            ...
            ($.isArray(this.option.value) && this.option.value.length>0)?(function(){
                ...
                value.forEach(function(val,index){
                    ...
                    for(var i=0;i<data.length;i++){
                        if(data[i].value==val){
                            arrr.push(i);
                            self.$cascaderList.find(".urp-cascader-child").eq(index).find("li").eq(i).trigger(self.triggerType,"initValue");
                            self.$cascaderList.find("ul").finish();      // 停止当前正在运行的动画
                        }
                    }
        ...
        callback: function () {
            ...
            // 每层点击时绑定事件
            self.$cascaderList.on(self.triggerType,".urp-cascader-child li",function(event,triggerData){
                ...
                ("children" in self.blockData)?(
                    ...
                    self.triggerType==="mouseenter"?(function() {
                        self.$cascaderList.find(".urp-cascader-child:gt("+(self.floor)+")").remove();
            ...
            $(self.elem).on("click", function () {
                ...
                zIndex++;
                if (self.onOff) {
                    self.$urpCascaderContent.slideDown(100);
                    if($.isFunction(self.option.onShow)){
                        self.option.onShow($(self.elem) , self.$urpCascaderContent);
                    }
                    self.$urpCascaderContent.find('i.layui-icon-down').replaceWith('<i class="layui-icon layui-icon-up"></i>');
                    self.$urpCascaderContent.css("zIndex",zIndex);
                } else {
                    self.$urpCascaderContent.slideUp(100);
                    self.$urpCascaderContent.find('i.layui-icon-up').replaceWith('<i class="layui-icon layui-icon-down"></i>');

                    if($.isFunction(self.option.onHide)){
                        self.option.onHide($(self.elem) , self.$urpCascaderContent);
                    }
                }
            });
            // 点击外层文档隐藏
